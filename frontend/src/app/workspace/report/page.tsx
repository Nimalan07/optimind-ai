...
export default function ReportPage() {
  ...
  useEffect(() => {
    ...
    if (savedResult) {
      try {
        setPipelineResult(JSON.parse(savedResult));
      } catch (e) {
  ...
  const latencyVal = pipelineResult?.benchmark?.after?.latency?.average_ms || 1.6;
  const memoryVal = pipelineResult?.benchmark?.after?.memory?.rss_mb || 2.5;
  const tputVal = pipelineResult?.benchmark?.after?.throughput?.requests_per_second || 36;
