...
export default function BenchmarkPage() {
  ...
  // Extract values
  const beforeLat = benchmarkResult?.before?.latency?.average_ms || 10.34;
  const afterLat = benchmarkResult?.after?.latency?.average_ms || 2.95;

  const beforeMem = benchmarkResult?.before?.memory?.rss_mb || 795;
  const afterMem = benchmarkResult?.after?.memory?.rss_mb || 357.75;

  const beforeTput = benchmarkResult?.before?.throughput?.requests_per_second || 96.6;
  const afterTput = benchmarkResult?.after?.throughput?.requests_per_second || 309.12;
